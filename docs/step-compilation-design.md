...
Evaluate step compilation on the same 4 SOP-Bench domains used in the
L1/L2 cache evaluation (see `cache_eval/2026-03-29/report.md`), using the
same SOP-consistent task exclusions.

### Domains

| Domain | Tasks | Exclusions |
|--------|:--:|---|
| dangerous_goods | 274 | None |
| video_classification | 197 | `exclusions/video_classification.json` |
| customer_service | 156 | None |
| traffic_spoofing_detection | 161 | `exclusions/traffic_spoofing_detection.json` (39 tasks) |

### Traces for compilation

Use the baseline traces from the cache evaluation as input to the compiler.
These are stored in `.proceda/runs/` and there are 150-275 completed runs
per domain — far more than the minimum 10 needed. Use N=10 traces per domain
(3 prompt examples + 7 held-out verification).

### Step 1: Compile all 4 domains

```bash
export OPENROUTER_API_KEY=$(pass soprun/OPENROUTER_API_KEY)

for domain in dangerous_goods video_classification customer_service traffic_spoofing_detection; do
    proceda compile "benchmarks/sop_bench/domains/${domain}/" \
        --num-traces 10 \
        --prompt-examples 3
done
```
...
- Which steps were successfully compiled (passed held-out verification)
- Which steps failed compilation (and why — language understanding, edge cases, codegen error)
- The compiled functions themselves (for review)

### Step 2: Run compiled benchmarks

```bash
EVAL_DATE=$(date +%Y-%m-%d)
EVAL_DIR="benchmarks/sop_bench/results/compile_eval/${EVAL_DATE}"
DATA_DIR=~/repos/3p/sop-bench/src/amazon_sop_bench/benchmarks/data

# Enable compilation in config
# cache:
#   enabled: true
#   optimization_level: 3   # or a new "compiled" mode

for domain in dangerous_goods video_classification customer_service traffic_spoofing_detection; do
    mkdir -p "${EVAL_DIR}/compiled/${domain}"

    python -m benchmarks.sop_bench.harness \
        --domain "${domain}" \
        --data-dir "${DATA_DIR}" \
        --exclude-tasks "benchmarks/sop_bench/exclusions/${domain}.json" \
        --output-dir "${EVAL_DIR}/compiled/${domain}"
done
```

### Step 3: Compare against baseline

Reuse baseline results from `cache_eval/2026-03-29/baseline/` — no need to
re-run the baseline since the same model configs and task exclusions apply.

```bash
BASELINE_DIR="benchmarks/sop_bench/results/cache_eval/2026-03-29/baseline"
```
...
| Domain | Baseline TSR | Compiled TSR | Delta | Steps Compiled | Steps on LLM |
|--------|:-----------:|:----------:|:-----:|:-------------:|:------------:|
| dangerous_goods | 94.9% | ? | ? | ?/7 | ?/7 |
| ... | | | | | |

**Token & LLM call savings:**

| Domain | BL Tok/Task | Compiled Tok/Task | Token Savings | BL Calls/Task | Compiled Calls/Task | Call Savings |
|--------|:-----------:|:-----------------:|:------------:|:------------:|:-------------------:|:-----------:|
| dangerous_goods | 28,409 | ? | ? | 10.9 | ? | ? |
| ... | | | | | | |

**Per-step compilation status:**

For each domain, show which steps compiled and which didn't:

| Domain | Step | Compiled? | Reason if failed | LLM calls saved |
|--------|:----:|:---------:|-----------------|:---------------:|
| dangerous_goods | 1 | ? | | 1 |
| dangerous_goods | 2 | ? | | 2 |
| ... | | | | |
...
Same as cache eval — diff per-task `is_correct` between baseline and compiled
to find regressions. Any task correct in baseline but wrong with compilation
needs investigation: was the compiled step wrong, or did the LLM on a
non-compiled step behave differently due to changed context?

### Step 5: Comparison with L1 caching

Since we have L1 results from the cache evaluation, include a 3-way comparison:

| Domain | Baseline TSR | L1 TSR | Compiled TSR | BL Tok/Task | L1 Tok/Task | Compiled Tok/Task |
|--------|:-----------:|:------:|:----------:|:-----------:|:-----------:|:-----------------:|
| dangerous_goods | 94.9% | 94.9% | ? | 28,409 | 30,653 | ? |
| ... | | | | | | |

This shows whether compilation achieves what L1 caching couldn't: actual
token and LLM call reduction without accuracy loss.

### Acceptance criteria

1. **Hard gate**: No domain may have TSR regression > 2% from baseline.
   (Same gate as the cache evaluation.)

2. **Token savings target**: For domains where > 50% of steps are compiled,
   expect > 40% token savings per task. If dangerous_goods compiles all 7
   steps, expect > 90% savings.

3. **Zero compiled-step errors on verified traces**: If a compiled function
   was verified against N traces, it must produce correct results on those
   traces at runtime. Any discrepancy is a verification bug.

4. **Fallback rate**: Track how often compiled steps fall back to the LLM at
   runtime (on inputs not seen during compilation). Target: < 5% fallback
   rate. Higher rates suggest the traces used for compilation weren't diverse
   enough.

### Trace and result storage

Same auditable structure as the cache evaluation:

```
benchmarks/sop_bench/results/compile_eval/
└── YYYY-MM-DD/
    ├── compiled/
    │   ├── dangerous_goods/
    │   │   ├── results.json
    ...
    │   ├── dangerous_goods_manifest.json
    │   ├── dangerous_goods_step_1.py
    │   ├── dangerous_goods_step_2.py
    │   └── ...
    ├── config_compiled.yaml
    └── report.md
```
